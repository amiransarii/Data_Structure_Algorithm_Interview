 ...
 *
 */
public class Test_94_Maximum_Profit_App {
public static int maximumProfit(int budget[]) {
		...
		for(int i = 0; i < n; i++){
			...
			if(count*budget[i] > max_profit){
				max_profit = count*budget[i];
				//price = budget[i]; it will return the which price needs to be set
			}
		}
      return max_profit;
	}
